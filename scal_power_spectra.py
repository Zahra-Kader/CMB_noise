...
Cl_ksz=np.load('/home/zahra/python_scripts/kSZ_21cm_signal/C_l_ksz_z_1_full_100pts_2d_2_1e5.npy')
ell_loaded=Cl_ksz[0,:]
Cl_ksz_full_arr=Cl_ksz[1,:]
OV_full_signal=interp1d(ell_loaded,Cl_ksz_full_arr)

print l_vec
plt.loglog(l_vec,l_vec**2*OV_full_signal(l_vec)/2/np.pi)
plt.show()
...
ell=np.geomspace(600,10000,100)

plt.loglog(ell,ell*(ell+1)*OV_full_signal(ell)/2./np.pi,'g')
plt.loglog(ell,ell*(ell+1)*(cl_tt_func_bis(ell)+nl_tt_ref_func_bis(ell))/2./np.pi,'r')
